...
import com.xiujichuanmei.a23_haoxin.adapter.UserWaitLetterStartAdapter;
...
/**
 * @author: X_Meteor
 * @description: 用户等信，发起Fragment
 * @version: V_1.0.0
 * @date: 2017/6/16 14:10
 * @company:
 * @email: lx802315@163.com
 */
public class UserWaitLetterSendFragment extends BaseFragment implements OnShowMyDemandListener, BaseQuickAdapter.RequestLoadMoreListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    UserWaitLetterStartAdapter adapter;
    ...
    protected View initLayout(LayoutInflater inflater, ViewGroup container, boolean b) {
        View rootView = inflater.inflate(R.layout.frag_user_find_letter_sub, null);
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        adapter = new UserWaitLetterStartAdapter(R.layout.item_frag_user_wait_start, list);
...
//                intent.putExtra("position", position);
                        ...
                        observable.compose(RxSchedulers.<BaseEntity<TimeInfo>>compose())
                                .subscribe(new BaseObserver<TimeInfo>(getActivity()) {
                                    ...
                                    protected void onHandleSuccess(TimeInfo timeInfo) {
                                        pop = new ShowTimePopupWindow(getActivity(), timeInfo, item.getOwenerHeadImg(), item.getOwnerName(), "");
    ...
    public void getLetters(int page) {
        model.getDemandList(new ShowMyLetterBody(Constants.USER_WAIT_LETTER_START, page, PAGE_SIZE, MyApplication.getInstance().getCurrentUser().getUserToken()), this);
    }

    @Override
    public void OnSearchSuccess(DemandInfo request, int page) {
        List<DemandInfo.MyDemandBean> letterList = request.getMyDemand();
